defmodule Membrane.S3.Sink do
  ...
  def handle_stopped_to_prepared(
    ...
    with {
         ...
         } <- ex_aws.request(init_op, aws_config) do
  ...
  def handle_prepared_to_stopped(
        _ctx,
        %{bucket: bucket, upload_id: upload_id, path: path, parts: parts, aws_config: aws_config, ex_aws: ex_aws} = state
      ) do
  ...
  def handle_write(
    ...
    %{headers: headers} = ExAws.S3.upload_part(
                            bucket,
                            path,
                            upload_id,
                            upload_index,
    ...
    {{:ok, demand: {:input, state.chunk_size}}, %{state | upload_index: upload_index + 1, parts: parts}}
  end
end
